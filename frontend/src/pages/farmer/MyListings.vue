          ...
          <div class="flex items-center gap-3.5">
            ...
            <div>
              <div class="flex items-center gap-2">
                ...
                <span class="text-[10px] font-extrabold px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-800 border border-emerald-200 flex items-center gap-1"><CheckCircle2 class="w-3 h-3 text-[#1E9444]" /> Verified Batch</span>
              </div>
              <p class="text-xs text-[#5A6270] mt-0.5 font-medium">{{ farmer?.region || 'Sidama' }} Region · {{ item.grade || 'Grade 1' }}</p>
            </div>
          </div>
          <span class="px-3 py-1 rounded-full text-xs font-black bg-[#EDFAF2] text-[#0F5C2A] border border-[#C3EFCF]">Live</span>
        </div>

        <div class="grid grid-cols-2 sm:grid-cols-3 gap-3 bg-white/80 p-3.5 rounded-2xl border border-orange-100/60 text-xs">
          <div><span class="text-[#5A6270] font-medium block">Price per Kg</span><span class="font-black text-[#1E9444] text-sm mt-0.5 block">{{ formatETB(item.pricePerKg) }}/kg</span></div>
          <div><span class="text-[#5A6270] font-medium block">Available Inventory</span><span class="font-black text-[#1E2328] text-sm mt-0.5 block">{{ item.availableQty?.toLocaleString() }} kg</span></div>
          <div class="col-span-2 sm:col-span-1"><span class="text-[#5A6270] font-medium block">Minimum Bulk Order</span><span class="font-bold text-[#1E2328] text-xs mt-0.5 block">{{ item.minOrderQty ? `${item.minOrderQty.toLocaleString()} kg` : '500 kg' }}</span></div>
        </div>
