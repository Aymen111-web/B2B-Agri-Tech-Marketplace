  ...
  <div class="w-full flex flex-col min-h-full pb-8 max-w-5xl mx-auto space-y-5">
    <div class="bg-gradient-to-r from-[#062E15] via-[#0F5C2A] to-[#0B57D0] text-white p-6 rounded-3xl shadow-sm relative overflow-hidden">
      ...
      <div class="flex items-center justify-between relative z-10">
        <div>
          <h1 class="text-xl sm:text-2xl font-black text-white tracking-tight">{{ $t('orders.farmerOrdersTitle') }}</h1>
          <p class="text-xs text-[#C3EFCF] mt-1 font-medium">{{ $t('orders.farmerOrdersSub') }}</p>
        </div>
      </div>
    </div>
    ...
    <div v-else class="space-y-4">
      <div v-for="order in orders" :key="order.id" class="bg-white border border-[#E2E4E7] rounded-2xl p-5 shadow-xs space-y-3">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 bg-[#F8F9FA] p-3 rounded-xl text-xs">
          <div><span class="text-[#5A6270] block font-medium">{{ $t('orders.crop') }}</span><span class="font-bold text-[#1E2328]">{{ $t(order.listing?.cropName) || $t('Produce') }}</span></div>
          <div><span class="text-[#5A6270] block font-medium">{{ $t('cart.quantity') }}</span><span class="font-bold text-[#1E2328]">{{ order.quantityKg?.toLocaleString() }} kg</span></div>
          <div><span class="text-[#5A6270] block font-medium">{{ $t('orders.amount') }}</span><span class="font-black text-[#1E9444]">{{ formatETB(order.totalAmountETB) }}</span></div>
          <div><span class="text-[#5A6270] block font-medium">{{ $t('Escrow Status') }}</span><span class="font-bold text-amber-600 capitalize">{{ $t(order.escrowStatus || 'held') }}</span></div>
        </div>
