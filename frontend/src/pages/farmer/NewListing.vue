<template>
  <div class="w-full flex flex-col min-h-full bg-[#F8F9FA] pb-28">
    <!-- Header -->
    <div class="px-4 py-3.5 bg-white border-b border-[#E2E4E7] flex items-center justify-between sticky top-14 z-30 shadow-2xs">
      <div class="flex items-center gap-3">
        ...
        <div>
          <h2 class="text-base font-black text-[#1E2328] tracking-tight">{{ $t('farmer.newListingTitle') }}</h2>
          <p class="text-[11px] text-[#5A6270]">Post new produce batch to B2B marketplace</p>
        </div>
      </div>
      <span class="px-2.5 py-1 rounded-full bg-[#EDFAF2] text-[#1E9444] border border-[#C3EFCF] text-[11px] font-bold flex items-center gap-1">
    ...
    <form @submit.prevent="handleSubmit" class="p-4 md:p-6 max-w-3xl mx-auto w-full space-y-6">
      <div v-if="submitError" class="p-4 bg-red-50 border border-red-200 rounded-2xl flex items-center gap-3 text-xs text-red-700 font-bold shadow-2xs">
      ...
      <div class="bg-white border border-[#E2E4E7] rounded-2xl p-5 shadow-2xs space-y-4">
        <div class="flex items-center justify-between border-b border-[#E2E4E7] pb-3">
          <div>
            <h3 class="text-sm font-black text-[#1E2328] uppercase tracking-wider flex items-center gap-2">
              <Camera class="w-4 h-4 text-[#1E9444]" />
              <span>{{ $t('farmer.uploadImages') }}</span>
            </h3>
            <p class="text-[11px] text-[#5A6270] mt-0.5">High quality photos attract 3x more buyer offers</p>
          </div>
          <span class="text-xs font-bold text-[#1E9444] bg-[#EDFAF2] px-2.5 py-0.5 rounded-full border border-[#C3EFCF]">
        ...
        <label 
          for="fileUploadBtn" 
          :class="[
            'border-2 border-dashed rounded-2xl p-6 text-center cursor-pointer block transition-all duration-200 bg-[#FAFBFD]',
            photoPreviews.length >= 5 ? 'opacity-50 pointer-events-none border-gray-200' : 'border-[#C3EFCF] hover:border-[#1E9444] hover:bg-[#F0FDF4]'
          ]"
        >
          <input 
            type="file" 
          ...
          <div class="flex flex-col items-center">
            <div class="w-12 h-12 rounded-2xl bg-[#EDFAF2] border border-[#C3EFCF] flex items-center justify-center mb-2.5">
              <UploadCloud class="w-6 h-6 text-[#1E9444]" />
            </div>
            <span class="text-sm font-bold text-[#1E2328] block">
              {{ isCompressing ? 'Compressing Produce Images...' : 'Tap or Drag produce photos here' }}
            </span>
            <span class="text-[11px] text-[#5A6270] mt-1">PNG, JPG, WebP up to 10MB per image</span>
          </div>
        </label>

        <!-- Loading State for Image Compression -->
        <div v-if="isCompressing" class="flex items-center justify-center gap-2 p-3 bg-blue-50 border border-blue-100 rounded-xl text-xs text-[#0B57D0] font-bold animate-pulse">
          <Loader2 class="w-4 h-4 animate-spin text-[#0B57D0]" />
        ...
        <div v-if="photoPreviews.length > 0" class="space-y-2 pt-2">
          <span class="text-[11px] font-bold text-[#5A6270] block uppercase tracking-wider">Photo Gallery Preview</span>
          <div class="grid grid-cols-3 sm:grid-cols-5 gap-3">
            <div 
              v-for="(src, i) in photoPreviews" 
              :key="i" 
              class="relative group rounded-xl overflow-hidden border-2 border-[#E2E4E7] aspect-square bg-gray-100 shadow-2xs"
            >
      ...
      <div class="bg-white border border-[#E2E4E7] rounded-2xl p-5 shadow-2xs space-y-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <!-- Category -->
          <div>
            <label class="text-[12px] font-bold text-[#1E2328] block mb-1">{{ $t('farmer.category') }} <span class="text-red-500">*</span></label>
            <select v-model="category" class="w-full px-3.5 py-2.5 bg-white border border-[#E2E4E7] rounded-xl text-sm focus:outline-none focus:border-[#1E9444] font-medium cursor-pointer">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <!-- Processing Method -->
          <div>
            <label class="text-[12px] font-bold text-[#1E2328] block mb-1">{{ $t('Process Method') }}</label>
            <select v-model="processMethod" class="w-full px-3.5 py-2.5 bg-white border border-[#E2E4E7] rounded-xl text-xs font-semibold focus:outline-none focus:border-[#1E9444] cursor-pointer">
      ...
      <div class="bg-white border border-[#E2E4E7] rounded-2xl p-5 shadow-2xs space-y-4">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            <label class="text-[12px] font-bold text-[#1E2328] block mb-1">{{ $t('farmer.minimumOrderQuantity') }} (KG) <span class="text-red-500">*</span></label>
            <input type="number" required min="1" v-model.number="minOrderQty" class="w-full px-3.5 py-2.5 bg-white border border-[#E2E4E7] rounded-xl text-sm font-bold focus:outline-none focus:border-[#1E9444]" />
          </div>
        </div>
        ...
        <div>
          ...
          <div class="mt-3 p-3.5 bg-[#EDFAF2] border border-[#C3EFCF] rounded-xl flex items-center justify-between text-xs">
      ...
      <div class="bg-white border border-[#E2E4E7] rounded-2xl p-5 shadow-2xs space-y-4">
        <h3 class="text-sm font-black text-[#1E2328] uppercase tracking-wider border-b border-[#E2E4E7] pb-3 flex items-center gap-2">
          <Calendar class="w-4 h-4 text-[#1E9444]" />
          <span>{{ $t('Quality details') }}</span>
        </h3>

        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label class="text-[12px] font-bold text-[#1E2328] block mb-1">{{ $t('farmer.harvestDate') }}</label>
            <input type="date" v-model="harvestDate" class="w-full px-3.5 py-2.5 bg-white border border-[#E2E4E7] rounded-xl text-xs font-semibold focus:outline-none focus:border-[#1E9444]" />
          </div>
          <div>
            <label class="text-[12px] font-bold text-[#1E2328] block mb-1">{{ $t('Moisture % (optional)') }}</label>
            <input type="number" step="0.1" v-model.number="moistureContent" placeholder="11.0" class="w-full px-3.5 py-2.5 bg-white border border-[#E2E4E7] rounded-xl text-xs font-medium focus:outline-none focus:border-[#1E9444]" />
          </div>
        </div>

        <div>
          <label class="text-[12px] font-bold text-[#1E2328] block mb-1">{{ $t('farmer.cropDescription') }}</label>
          <textarea rows="4" v-model="description" placeholder="Describe crop origin, cupping notes, altitude, moisture grade..." class="w-full px-3.5 py-2.5 bg-white border border-[#E2E4E7] rounded-xl text-xs font-medium min-h-[100px] focus:outline-none focus:border-[#1E9444]" />
        </div>
      </div>

      <!-- Sticky Submit Bar -->
      <div class="fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-[#E2E4E7] p-4 max-w-3xl mx-auto shadow-lg">
        <button 
          type="submit" 
