  ...
  <div class="w-full flex flex-col min-h-full pb-8 max-w-5xl mx-auto space-y-5">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
      <div class="bg-white border border-[#E2E4E7] rounded-2xl p-4 shadow-xs relative overflow-hidden flex flex-col justify-between space-y-3">
        ...
        <div class="flex items-start justify-between">
          <div><span class="text-xs font-bold text-[#5A6270]">{{ $t('farmer.activeProduceListings') }}</span><h3 class="text-2xl font-black text-[#1E2328] mt-1">{{ activeCount }}</h3></div>
        ...
        <span class="text-[10px] font-bold text-[#1E9444]">{{ $t('farmer.liveOnMarketplace') }}</span>
      </div>

      <div class="bg-white border border-[#E2E4E7] rounded-2xl p-4 shadow-xs relative overflow-hidden flex flex-col justify-between space-y-3">
        <div class="absolute top-0 left-0 right-0 h-1 bg-[#F5B73A]" />
        <div class="flex items-start justify-between">
          <div><span class="text-xs font-bold text-[#5A6270]">{{ $t('farmer.regionalDepots') }}</span><h3 class="text-2xl font-black text-[#1E2328] mt-1">0</h3></div>
        ...
        <span class="text-[10px] font-bold text-amber-700">{{ $t('farmer.supportedLogistics') }}</span>
      </div>

      <div class="bg-white border border-[#E2E4E7] rounded-2xl p-4 shadow-xs relative overflow-hidden flex flex-col justify-between space-y-3">
        <div class="absolute top-0 left-0 right-0 h-1 bg-[#E6533C]" />
        <div class="flex items-start justify-between">
          <div><span class="text-xs font-bold text-[#5A6270]">{{ $t('farmer.primaryUnionCoops') }}</span><h3 class="text-2xl font-black text-[#1E2328] mt-1">{{ farmer?.union ? 1 : 0 }}</h3></div>
        ...
        <span class="text-[10px] font-bold text-red-600">{{ $t(farmer?.union) || $t('None') }}</span>
      </div>

      <div class="bg-white border border-[#E2E4E7] rounded-2xl p-4 shadow-xs relative overflow-hidden flex flex-col justify-between space-y-3">
        <div class="absolute top-0 left-0 right-0 h-1 bg-[#14B8A6]" />
        <div class="flex items-start justify-between">
          <div><span class="text-xs font-bold text-[#5A6270]">{{ $t('Verified Buyers') }}</span><h3 class="text-2xl font-black text-[#1E2328] mt-1">0</h3></div>
        ...
        <span class="text-[10px] font-bold text-teal-700">{{ $t('Commercial Escrow Buyers') }}</span>
      </div>

      <div class="bg-white border border-[#E2E4E7] rounded-2xl p-4 shadow-xs relative overflow-hidden flex flex-col justify-between space-y-3">
        <div class="absolute top-0 left-0 right-0 h-1 bg-amber-400" />
        <div class="flex items-start justify-between">
          <div><span class="text-xs font-bold text-[#5A6270]">{{ $t('Orders Received') }}</span><h3 class="text-2xl font-black text-[#1E2328] mt-1">{{ receivedOrdersCount }}</h3></div>
        ...
        <span class="text-[10px] font-bold text-amber-700">{{ $t('Pending Escrow Release:') }} {{ formatETB(pendingPayoutETB) }}</span>
      </div>

      <div class="bg-white border border-[#E2E4E7] rounded-2xl p-4 shadow-xs relative overflow-hidden flex flex-col justify-between space-y-3">
        <div class="absolute top-0 left-0 right-0 h-1 bg-[#E6533C]" />
        <div class="flex items-start justify-between">
          <div><span class="text-xs font-bold text-[#5A6270]">{{ $t('SMS Dispatch Notifications') }}</span><h3 class="text-2xl font-black text-[#1E2328] mt-1">0</h3></div>
        ...
        <span class="text-[10px] font-bold text-red-600">{{ $t('Active Mobile SMS Channel') }}</span>
      </div>
    </div>
    ...
    <div v-if="orders.length > 0 && orders[0].status === 'in_transit'" @click="$router.push('/farmer/orders')"
      ...
      <div class="flex items-center gap-3">
        ...
        <div>
          <div class="flex items-center gap-2">
            <h4 class="text-xs font-extrabold text-[#0F5C2A]">{{ $t('orders.orderId') }} #{{ orders[0].id }} {{ $t('in_transit') }}</h4>
            <span class="px-2 py-0.5 rounded-full bg-[#1E9444] text-white text-[10px] font-extrabold">{{ $t('orders.paymentSecured') }}</span>
          </div>
          <p class="text-[11px] text-[#5A6270] mt-0.5 font-medium">{{ $t('Batch on its way to buyer hub.') }}</p>
        </div>
      </div>
    ...
    <div class="bg-white border border-[#E2E4E7] rounded-2xl p-5 shadow-xs space-y-4">
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-gray-100 pb-3">
        <div class="flex items-center gap-2">
          ...
          <h3 class="text-sm sm:text-base font-black text-[#1E2328]">{{ $t('Produce & Order Status') }}</h3>
          <span class="text-[11px] font-bold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded-md border border-emerald-200">• {{ $t('• Live just now') }}</span>
        </div>
        ...
        <div class="flex items-center gap-2">
          <router-link to="/farmer/listings" class="px-3.5 py-1.5 rounded-xl border border-gray-200 bg-[#F8F9FA] hover:bg-gray-100 text-xs font-bold text-[#1E2328] transition-colors flex items-center gap-1 cursor-pointer">
            <span>{{ $t('See All Listings') }}</span><ChevronRight class="w-3.5 h-3.5 text-[#1E9444]" />
          </router-link>
      ...
      <div v-else class="space-y-2.5">
        <div v-for="item in displayListings" :key="item.id" @click="$router.push(`/farmer/listings/edit/${item.id}`)"
          ...
          <div class="flex items-center gap-3">
            ...
            <div>
              <h4 class="text-xs font-black text-[#1E2328]">{{ $t(item.cropName) }}</h4>
              <p class="text-[11px] text-[#5A6270] mt-0.5">{{ $t(farmer?.region) || 'Addis Ababa' }} {{ $t('Region · Verified Harvest') }}</p>
            </div>
          </div>
          <div class="text-right flex items-center gap-4">
            <div>
              <span class="text-xs font-black text-[#1E2328] block">{{ formatETB(item.pricePerKg) }}/{{ $t('kg') }}</span>
              <span class="text-[10px] font-bold text-[#1E9444]">{{ item.availableQty?.toLocaleString() }} {{ $t('kg available') }}</span>
            </div>
            <span class="hidden sm:inline-block px-2.5 py-1 rounded-full text-[10px] font-black bg-[#EDFAF2] text-[#0F5C2A] border border-[#C3EFCF]">{{ $t('badges.live') }}</span>
          </div>
        </div>
      </div>
    </div>
