  ...
  <div class="space-y-6 lg:space-y-8 animate-in fade-in duration-500">
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-1 gap-5 relative z-10">
      <article v-for="user in users" :key="user.id" 
        :class="['bg-white border lg:border-transparent lg:border-b lg:border-b-[#E2E4E7] lg:rounded-none rounded-2xl p-5 lg:p-6 shadow-sm lg:shadow-none transition-all group lg:flex lg:items-center lg:justify-between lg:gap-8',
          user.account_status === 'suspended' ? 'opacity-80 lg:hover:bg-red-50 hover:border-red-200' : 'lg:hover:bg-[#F8F9FA] hover:shadow-xl hover:border-[#E2E4E7]']">
        
        <div class="flex items-start gap-4 lg:gap-5 lg:w-[45%]">
          <div class="relative">
            <div class="w-14 h-14 rounded-full bg-[#F8F9FA] border border-[#E2E4E7] flex items-center justify-center text-xl font-black shrink-0 shadow-sm overflow-hidden text-indigo-800 bg-gradient-to-br from-indigo-50 to-violet-100">
            ...
            <div v-if="user.account_status === 'suspended'" class="absolute -bottom-1 -right-1 w-5 h-5 rounded-full bg-red-100 flex items-center justify-center border-2 border-white shadow-2xs">
              <Ban class="w-3 h-3 text-red-600" />
            </div>
          </div>
          
          <div>
            <div class="flex flex-wrap sm:items-center gap-2">
              <h3 class="text-[17px] font-extrabold text-[#1E2328] group-hover:text-indigo-700 transition-colors">
