<template>
  <div class="space-y-6 lg:space-y-8 animate-in fade-in duration-500">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-5 border-b border-[#E2E4E7] pb-6 relative">
      <div class="relative z-10">
        <div class="flex items-center gap-3 mb-1.5">
          ...
          <h1 class="text-[28px] font-black text-[#1E2328] tracking-tight">{{ $t('admin.disputeTitle') }}</h1>
        </div>
      ...
      <div class="flex items-center gap-3 z-10">
        <div class="flex items-center bg-white border border-[#E2E4E7] rounded-xl p-1 shadow-xs">
          <span class="px-3 py-1.5 text-[11px] font-black uppercase text-[#1E2328]">{{ $t('admin.activeClaims') }}</span>
    ...
    <div v-if="isLoading" class="flex flex-col items-center justify-center py-20">
      ...
      <p class="text-xs font-black text-[#1E2328] uppercase tracking-wider mt-2">{{ $t('admin.syncingExceptions') }}</p>
      <p class="text-[11px] text-[#9BA1AA] font-bold mt-1">{{ $t('admin.retrievingAnomalyReports') }}</p>
    </div>
    ...
    <div v-else-if="disputes.length === 0" class="text-center py-20 bg-white border border-[#E2E4E7] rounded-[24px] px-8 shadow-sm relative overflow-hidden group">
      ...
      <div class="relative z-10">
        ...
        <h3 class="text-[18px] font-extrabold text-[#1E2328] tracking-tight">{{ $t('admin.noActiveDisputes') }}</h3>
      ...
      <article v-for="d in disputes" :key="d.id" 
        ...
        <div>
          ...
          <div class="py-4">
            <div class="grid grid-cols-2 gap-4 mb-4 text-[12px]">
              <div>
                <span class="block font-bold text-[#9BA1AA] uppercase tracking-wider text-[10px] mb-1">{{ $t('admin.claimType') }}</span>
                <span class="font-bold text-[#1E2328] capitalize">{{ d.type ? $t(d.type) : $t('admin.generalException') }}</span>
              </div>
              <div>
                <span class="block font-bold text-[#9BA1AA] uppercase tracking-wider text-[10px] mb-1">{{ $t('admin.filedBy') }}</span>
                <span class="font-bold text-[#1E2328]">{{ d.raised_user?.first_name ? (d.raised_user.first_name + ' ' + (d.raised_user.second_name || '')) : $t('admin.systemAutoFlag') }}</span>
              </div>
            </div>
            ...
            <div class="bg-rose-50/50 p-4 rounded-xl border border-rose-100 relative overflow-hidden">
              ...
              <span class="block font-bold text-[#9BA1AA] uppercase tracking-wider text-[9px] mb-1">{{ $t('admin.incidentDetailReport') }}</span>
        ...
        <div class="flex flex-col sm:flex-row justify-end gap-2 pt-4 border-t border-gray-100 mt-auto">
          <button @click="resolveDispute(d.id, 'refund_buyer')" 
            class="flex-1 sm:flex-none px-5 py-2.5 rounded-xl border-2 border-orange-100 text-orange-700 text-[12px] font-bold hover:bg-orange-50 hover:border-orange-200 transition-colors shadow-2xs focus:ring-4 focus:ring-orange-100 cursor-pointer">
            {{ $t('admin.refundBuyer') }}
          </button>
