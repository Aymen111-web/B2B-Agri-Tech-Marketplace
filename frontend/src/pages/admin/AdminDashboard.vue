<template>
  <div class="space-y-6 lg:space-y-8 animate-in fade-in duration-500">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-5 border-b border-[#E2E4E7] pb-6 relative">
      <div class="relative z-10">
        <div class="flex items-center gap-3 mb-1.5">
          ...
          <h1 class="text-[28px] font-black text-[#1E2328] tracking-tight">Executive Command Center</h1>
        </div>
        <p class="text-[14px] font-medium text-[#5A6270] max-w-xl">
          Real-time B2B marketplace monitoring, verification queue, escrow audit, and security overview.
        </p>
      </div>
      <div class="flex items-center gap-3 z-10 mt-4 sm:mt-0">
        ...
        <button @click="loadDashboardData" :disabled="isLoading" 
          class="px-4 py-2 rounded-xl border border-[#E2E4E7] bg-white text-[13px] font-bold text-[#1E2328] hover:bg-[#F8F9FA] hover:shadow-md transition-all active:scale-95 flex items-center gap-2 group">
    ...
    <div v-if="isLoading" class="flex flex-col items-center justify-center py-20">
      ...
      <p class="text-xs font-black text-[#1E2328] uppercase tracking-wider mt-2">Connecting to Data Grid</p>
      <p class="text-[11px] text-[#9BA1AA] font-bold mt-1">Aggregating live marketplace metrics...</p>
    </div>
    ...
    <template v-else>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        <div class="bg-white border border-[#E2E4E7] rounded-3xl p-6 shadow-sm hover:shadow-xl transition-all group flex flex-col justify-between overflow-hidden relative">
          ...
          <div class="pt-4 mt-6 border-t border-[#E2E4E7]/60 flex items-center justify-between text-[12px] font-bold text-[#5A6270] relative z-10">
            <span class="flex items-center gap-1.5"><TrendingUp class="w-4 h-4 text-[#1E9444]" /> {{ kpis?.total_orders || 0 }} Total Orders</span>
            <router-link to="/admin/orders" class="text-[#1E9444] hover:underline flex items-center gap-1">View Orders <ArrowRight class="w-3 h-3 group-hover:translate-x-1 transition-transform"/></router-link>
          </div>
        </div>
        ...
        <div class="bg-white border border-[#E2E4E7] rounded-3xl p-6 shadow-sm hover:shadow-xl transition-all group flex flex-col justify-between overflow-hidden relative">
          ...
          <div class="relative z-10">
            <div class="flex items-center justify-between text-[#5A6270] mb-4">
              <span class="text-[11px] font-extrabold uppercase tracking-wider text-amber-700">Verification</span>
          ...
          <div class="pt-4 mt-6 border-t border-[#E2E4E7]/60 flex items-center justify-between text-[12px] font-bold text-[#5A6270] relative z-10">
            <span class="text-amber-700 font-semibold flex items-center gap-1.5"><AlertTriangle class="w-4 h-4" /> Action Required</span>
            <router-link to="/admin/applications" class="text-amber-800 hover:underline flex items-center gap-1">Review <ArrowRight class="w-3 h-3 group-hover:translate-x-1 transition-transform"/></router-link>
          </div>
        </div>
        ...
        <div class="bg-white border border-[#E2E4E7] rounded-3xl p-6 shadow-sm hover:shadow-xl transition-all group flex flex-col justify-between overflow-hidden relative">
          ...
          <div class="relative z-10">
            <div class="flex items-center justify-between text-[#5A6270] mb-4">
              <span class="text-[11px] font-extrabold uppercase tracking-wider text-blue-700">Active Listings</span>
          ...
          <div class="pt-4 mt-6 border-t border-[#E2E4E7]/60 flex items-center justify-between text-[12px] font-bold text-[#5A6270] relative z-10">
            <span class="text-blue-700 font-semibold flex items-center gap-1.5"><ShoppingCart class="w-4 h-4" /> Live Market</span>
            <router-link to="/admin/listings" class="text-blue-700 hover:underline flex items-center gap-1">Moderate <ArrowRight class="w-3 h-3 group-hover:translate-x-1 transition-transform"/></router-link>
          </div>
        </div>
        ...
        <div class="bg-white border border-[#E2E4E7] rounded-3xl p-6 shadow-sm hover:shadow-xl transition-all group flex flex-col justify-between overflow-hidden relative">
          ...
          <div class="relative z-10">
            <div class="flex items-center justify-between text-[#5A6270] mb-4">
              <span class="text-[11px] font-extrabold uppercase tracking-wider text-rose-700">Escrow Disputes</span>
          ...
          <div class="pt-4 mt-6 border-t border-[#E2E4E7]/60 flex items-center justify-between text-[12px] font-bold text-[#5A6270] relative z-10">
            <span class="text-rose-700 font-semibold flex items-center gap-1.5"><Lock class="w-4 h-4" /> Funds Held</span>
            <router-link to="/admin/disputes" class="text-rose-700 hover:underline flex items-center gap-1">Arbitrage <ArrowRight class="w-3 h-3 group-hover:translate-x-1 transition-transform"/></router-link>
          </div>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-0 bg-white border border-[#E2E4E7] p-1.5 rounded-[24px] shadow-sm mt-8">
        <div class="flex items-center gap-4 p-4 hover:bg-[#F8F9FA] rounded-2xl transition-colors cursor-default">
          ...
          <div><span class="text-[11px] font-black text-[#9BA1AA] uppercase tracking-wider">Verified Ecosystem</span><p class="text-[15px] font-extrabold text-[#1E2328] mt-0.5">{{ kpis?.total_farmers || 0 }} Farmers · {{ kpis?.total_buyers || 0 }} Buyers</p></div>
        </div>
        <div class="flex items-center gap-4 p-4 border-t sm:border-t-0 sm:border-l border-[#E2E4E7]/60 hover:bg-[#F8F9FA] rounded-2xl transition-colors cursor-default">
          ...
          <div><span class="text-[11px] font-black text-[#9BA1AA] uppercase tracking-wider">Farmer Payouts</span><p class="text-[15px] font-extrabold text-[#1E2328] mt-0.5">{{ formatETB(kpis?.pending_payouts_amount || 0) }} pending</p></div>
        </div>
        <div class="flex items-center gap-4 p-4 border-t sm:border-t-0 sm:border-l border-[#E2E4E7]/60 hover:bg-[#F8F9FA] rounded-2xl transition-colors cursor-default">
          ...
          <div><span class="text-[11px] font-black text-[#9BA1AA] uppercase tracking-wider">System Orders</span><p class="text-[15px] font-extrabold text-[#1E2328] mt-0.5">{{ kpis?.total_orders || 0 }} Completed</p></div>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 mt-8">
        <div class="lg:col-span-7 bg-white border border-[#E2E4E7] rounded-3xl shadow-sm overflow-hidden flex flex-col">
          <div class="p-6 border-b border-[#E2E4E7]/60 flex items-center justify-between bg-gradient-to-r from-[#F8F9FA] to-white">
            <div>
              <h2 class="text-[17px] font-extrabold text-[#1E2328] tracking-tight">Pending Action Queue</h2>
              <p class="text-[12px] font-semibold text-[#9BA1AA] mt-1">1-Click capability request approvals.</p>
            </div>
            <router-link to="/admin/applications" class="text-[12px] font-bold text-[#1E9444] hover:bg-[#EDFAF2] px-3 py-1.5 rounded-lg border border-transparent hover:border-[#C3EFCF] transition-all">View All ({{ kpis?.pending_applications || 0 }})</router-link>
          </div>
          ...
          <div v-if="pendingApprovals.length === 0" class="p-12 text-center text-[#5A6270] my-auto flex flex-col items-center">
            ...
            <h3 class="text-[16px] font-extrabold text-[#1E2328]">Clear Queue</h3>
            <p class="text-[13px] font-medium mt-1">No pending verification items require your attention right now.</p>
          </div>
              ...
              <div class="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p class="text-[12px] font-medium text-[#5A6270] flex items-center gap-2">
                    ...
                    <span>Received: {{ formatDate(app.created_at) }}</span>
                  </p>
                </div>
              </div>
              <div class="flex items-center gap-2 opacity-0 lg:opacity-100 lg:group-hover:opacity-100 transition-opacity">
                <button @click="handleQuickApprove(app.id)" class="px-4 py-2 rounded-xl bg-gradient-to-tr from-[#1E9444] to-emerald-500 text-white font-bold text-[12px] hover:from-[#0F5C2A] flex items-center gap-1.5 shadow-md shadow-emerald-500/20 active:scale-95 transition-all"><Check class="w-3.5 h-3.5" /> Approve</button>
                <button @click="handleQuickReject(app.id)" class="px-4 py-2 rounded-xl bg-white border border-rose-200 text-rose-600 font-bold text-[12px] hover:bg-rose-50 active:scale-95 transition-all shadow-sm">Reject</button>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="lg:col-span-5 bg-white border border-[#E2E4E7] rounded-3xl shadow-sm overflow-hidden flex flex-col relative z-20">
          <div class="p-6 border-b border-[#E2E4E7]/60 flex items-center justify-between bg-gradient-to-r from-gray-50 to-white">
            <div class="flex items-center gap-2.5">
              ...
              <div>
                <h2 class="text-[17px] font-extrabold text-[#1E2328] tracking-tight">Audit Trail Log</h2>
                <p class="text-[11px] font-black uppercase text-[#9BA1AA] tracking-wider mt-0.5">Automated Event Tracking</p>
              </div>
            </div>
          </div>
          <div class="p-4 overflow-y-auto h-[450px] relative scrollbar-hide">
            <p v-if="recentActivity.length === 0" class="text-center text-xs font-bold text-[#9BA1AA] py-12">No recent event trail items.</p>
            ...
            <div class="relative z-10 flex flex-col gap-6 p-2">
              <div v-for="log in recentActivity" :key="log.id" class="flex items-start gap-4 text-sm group">
                ...
                <div class="flex-1 min-w-0 bg-[#F8F9FA] rounded-2xl p-4 border border-[#E2E4E7]/60 group-hover:border-indigo-200 transition-colors">
                  ...
                  <p class="text-[12px] font-medium text-[#5A6270] truncate leading-relaxed">By: {{ log.user?.name || 'System Auto' }} <br/> Resource: <span class="font-bold text-indigo-700 bg-indigo-50 px-1 py-0.5 rounded">{{ log.auditable_type || 'Unknown' }} #{{ log.auditable_id }}</span></p>
                </div>
              </div>
            </div>
          </div>
...
const handleQuickApprove = async (id) => {
  try {
    ...
    actionMessage.value = `Verification Approved!`
...
const handleQuickReject = async (id) => {
  ...
  try {
    ...
    actionMessage.value = `Verification Denied.`
    setTimeout(() => actionMessage.value = null, 4000)
    loadDashboardData()
  } catch (err) {
